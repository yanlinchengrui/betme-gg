...
class App extends Component {
  ...
  getUpcomingMatches = () => {
    axios
      .get(`https://api.pandascore.co/matches/upcoming?token=${process.env.REACT_APP_TOKEN}`)
      .then(rez => {
        console.log("--------xxx-----", rez.data);
        this.setState({ upcomingMatches: rez.data.slice(0, 5) });
      });
  }
